...
class SAM3Orchestrator {
  ...
  /**
   * Ensure the GPU is available for YOLO training by unloading SAM3 if needed.
   *
   ...
   *
   * @param timeoutMs Maximum time to wait for SAM3 unload (default 30 seconds)
  ...
  async ensureGPUAvailable(timeoutMs: number = 30000): Promise<{ success: boolean; message: string }> {
    ...
    if (status.instanceState === 'stopped') {
      console.log('[Orchestrator] SAM3 instance stopped, GPU available for YOLO');
      return { success: true, message: 'SAM3 instance stopped' };
    }

    if (!status.modelLoaded) {
      console.log('[Orchestrator] SAM3 model not loaded, GPU available for YOLO');
      return { success: true, message: 'SAM3 model not loaded' };
    }

    console.log('[Orchestrator] Unloading SAM3 model for YOLO training...');
    const result = await awsSam3Service.unloadModel(timeoutMs);
    if (result.success) {
